...
const mountModel = async (Mounted: Mounted, type: string, boxWidth: number, boxHeight: number, boxDepth: number, boxScale: number): Promise<THREE.Group | null> => {
    ...
    const planFront = svgString ?  await svgLoader(svgString) : null;
    ...
    if(Mounted.OtherSideSVGFile !== null)
    {
        ...
        const planRear = svgString ? await svgLoader(svgString) : null;
        if(planRear)
        {
...
const createDetailBoxesTop = (
    ...
    mesh.receiveShadow = true;      
    // Position the mesh
    mesh.position.y = positionY/ 2 + 75;
    // Assign a unique name
    mesh.name = `Detail Data ${index === 0 ? "Top" : "Bottom"}`;
    return mesh;   
};
export async function createFacility(facilityInfo: JsonType[] | null): Promise<THREE.Group<THREE.Object3DEventMap> | null>{
    if(!facilityInfo)
    {
        return null;
    }
    const boxGroup: THREE.Group<THREE.Object3DEventMap> = new THREE.Group();
    let viewWidth = 0;
    let viewHeight = 0;
    let viewDepth = 0;
    let countSvg = 0;
    for (const [parentIndex, item] of facilityInfo.entries()) {
        ...
        if(group)
        {
            ...
            if (item.View === "Front") {
                const detail = createDetailBoxesTop(item.Details[0], 0, viewHeight);
                boxGroup.add(detail);
                group.scale.y *= -1;
